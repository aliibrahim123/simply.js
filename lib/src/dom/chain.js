...
class CDom {
	constructor (sel, root){
		...
		if (this.els.length === 0) console.warn('empty chain dom')
	}
	static extend (obj) {
	...
	add (sel) {
		var el = $el(sel);
		Array.isArray(el) ? this.els.push(...el) : this.els.push(el);
		return this
	}
	get cloneSelf () {
	...
	clean () {//filter elements not in document
		this.els = this.els.filter(el => $doc.body.contains(el));
		return this
	}
	get isInDom () {
		return $doc.body.contains(this.els[0])
	}
	get reverse () {
