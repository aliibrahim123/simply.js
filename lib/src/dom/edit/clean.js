...
export var cleanDom = (node, doNotCleanTags = [], doNotMergeTags = [], tagset = new Set()) => {
	checknode(node, 'node');
	...
	if (node.nodeType === node.ELEMENT_NODE) {
		...
		if (doNotCleanTags.includes(tag)) {}
		...
		children.forEach(child => cleanDom(child, doNotCleanTags, doNotMergeTags, new Set([...tagset, tag])))
	} 
