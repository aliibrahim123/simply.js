...
var useBindActs = {
	...
	'bind:if' (comp, act) {
		comp.binder.addBind(act.prop, {type: 'if', el: act.el[0], prop: act.prop, condF: act.condF, fun: act.fun, args: act.args})
	},
	'bind:if-else' (comp, act) {
		comp.binder.addBind(act.prop, {type: 'ifElse', el: act.el[0], prop: act.prop, conds: act.conds})
	},
	'bind:for' (comp, act) {
