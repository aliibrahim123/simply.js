...
var handlers = {
	...
	for (obj, val, comp,_, prop,__,meta) {//array looping
		...
		if (!meta.arr) { //no array
			obj.el.replaceChildren(...obj.onemptyEls)
			if (prop) prop.meta['arr:old'] = undefined;
		} else if (meta['arr:empty']) {//empty array
			obj.el.replaceChildren(...obj.onemptyEls);
			if (obj.mtype === 'collection') obj.collection.remove(0, obj.collection.comps.length) //remove all components in collection
		} else if (meta['arr:wasempty']) {//was empty
			obj.el.replaceChildren();
			...
			if (obj.type === 'collection') obj.collection.set(val)
		}
	}
}
