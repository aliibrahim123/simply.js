	...
	methods: {
		...
		'if' (comp, el, value, prop, attr) {
			...
			comp.doAct({
				...
				el, 
				prop, 
				condF: Function('comp', 'el', prop, args[0].includes(';') ? args[0] : 'return ' + args[0]), 
				fun: args[1], 
				args: args.slice(2)
			});
			el.removeAttribute(attr);
		},
		'if-else' (comp, el, value, prop, attr) {
			var args = JSON.parse(value)
			comp.doAct({
				type: 'bind:if-else', 
				el, 
				...
				conds: args.map(i=> { return {
					condF: Function('comp', 'el', ...prop, i[0].includes(';') ? i[0] : 'return ' + i[0]),
					fun: i[1],
					args: i.slice(2)
				}})
			});
