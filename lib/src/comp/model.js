...
class Model {
	...
	constructor (comp, opts = {}, data = {}) {
		if (!comp?.isComp) throw new CompError('model: component of type (' + comp?.constructor?.name + '), expected (@componnt)');
		this.comp = comp;
	...
	add (name, item, returnProp = false) {//low level + reactivity 
		...
		else return prop ? prop.value : this
	}
	propagate (name, prop = this.data[name], oldValue, newValue = prop.value, meta = {}) {//reactivity core
		if (!prop) this.comp.warn('model: propagating undefined property (' + name + ')');
			...
			this.comp.trigger('model:propag', this, name, prop, oldValue, newValue, meta); //trigger reactivity
			if (meta.propagDeps !== false && this.deps[name]) this.deps[name].forEach(p => this.propagate(p, undefined,undefined,undefined, {propagDeps: false, force: true}));
			if (this.opts.propagParent && newValue !== prop.value) this.propagate(prop.name, prop, undefined,undefined, {force: true, sinceChild: true}) 
		}
	}
	getLow (name, doset = false /* adding objects till end */) {//path name handling
		...
		if (!prop && doset === false) {
			...
			return [undefined, undefined, undefined, path, isNew];
		}
		var isNew = false;
		if (!prop && doset) (prop = this.addLow(path[0], {})) && (isNew = true);
		var curObj = prop.value;
		var curPath;
	...
	setLow (name, value) { 
		...
		if (prop.setter) prop.setter(value, path, comp);
		else if (path.length === 1) prop.value = value;
		else parObj[path[path.length -1]] = value;
		return [prop, isNew ? undefined : oldVal, value]
	}
	set (name, value, returnProp = false, meta = {}) {
		...
		if (prop.isReactive && this.opts.propagate) this.propagate(name, prop, oldVal, newVal, meta);
		return returnProp ? prop : this
	}
	get (name, returnProp = false) {
		var [value, prop, parObj, path] = this.getLow(name, false);
		if (!prop) return undefined;
		return returnProp ? prop : prop.getter ? prop.getter(path) : value
	}
	getFn(name, fn, ...args) {
		var value = this.get(name);
		var Fn = typeof fn === 'function' ? fn : this.funs[fn];
		if (!Fn) throw new CompError('model: undefined function (' + fn + ')');
		var fnVal = Fn(this, value, ...args);
	...
	setFn(name, fn, old = true, ...args) {
		...
		var Fn = typeof fn === 'function' ? fn : this.funs[fn];
		if (!Fn) throw new CompError('model: undefined function (' + fn + ')');
		var fnVal = old ? Fn(this, value, ...args) : Fn(this, ...args);
		var rVal = fnVal?.$hasMeta ? fnVal.value : fnVal;
		this.set(name, rVal, false, fnVal?.meta);
		return this
	}
	has (name) {
