...
var simple = (oldarr, newarr, changeLev = 3, addLev = 5, getValue = (v) => v) => {
	...
	while (ni !== nlen && oi !== olen) {//loop till the end of either the old or new array
		...
		if (oitem === nitem) { //if same, continue to next
			oi++;
			ni++;
			continue
		}
		while (dl !== changeLev) { //may be a change (loop through both)
			dl++;
			if (oi + dl === olen && ni + dl === nlen) {//if ended at both the same time, push change then break the whole
				diffarr.push({type: 'change', ind: oi + dv, count: dl, nind: ni});
				oi = olen;//force full breaking
				ni = nlen;
				contLoop = true;
				break
			}
		...
		dl = 0;
		while (dl !== addLev) { //may be an add (loop through new)
			dl++;
