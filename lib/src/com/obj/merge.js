...
export var mergeDeep = (base, layer, copyfn = false) => {
	...
	for (let prop in layer) {
		toreturn[prop] = mergeDeep(base[prop], layer[prop])
	}
	if (copyfn) for (let prop in base) {//copy by function the properties not found in layer
		if (layer[prop] === undefined) toreturn[prop] = copyfn(base[prop]);
	}
	return toreturn
};

export var mergeMap = (base, layer, map, copyfn = false) => {
	...
	for (let prop in layer) {
		if (map[prop] !== false) toreturn[prop] = mergeMap(base[prop], layer[prop], map[prop], copyfn)
	}
	if (copyfn) for (let prop in base) {//copy by function the properties not found in layer
		if (layer[prop] === undefined) toreturn[prop] = copyfn(base[prop]);
	}
